...
const imp_label = Array.from(document.getElementsByClassName('imperial-label'));
const metric_label = Array.from(document.getElementsByClassName('metric-label'));
...
function handle_toggle_input(){
    if(imperial_radio.checked){
        ...
        for(let i = 0; i < imp_label.length; i++){
            imp_label[i].style.display = 'inline';
            metric_label[i].style.display = 'none';
        }

        //disable metric fields
        document.querySelectorAll('#metric-container input, #metric-cut-container input, #metric-container select, #metric-cut-container select').forEach(el =>{
            el.disabled = true;
            el.removeAttribute('required');
        });

        //enable imperial fields
        document.querySelectorAll('#imperial-container input, #imperial-cut-container input, #imperial-container select, #imperial-cut-container select').forEach(el =>{
            el.disabled = false;
        ...
        for(let i = 0; i < metric_label.length; i++){
            imp_label[i].style.display = 'none';
            metric_label[i].style.display = 'block';
        }

        //disable imperial fields
        document.querySelectorAll('#imperial-container input, #imperial-cut-container input, #imperial-container select, #imperial-cut-container select').forEach(el =>{
            el.disabled = true;
            el.removeAttribute('required');
        });

        //enable metric fields
        document.querySelectorAll('#metric-container input, #metric-cut-container input, #metric-container select, #metric-cut-container select').forEach(el =>{
            el.disabled = false;
            el.setAttribute('required', 'required');
        });
...
//adds imperial board to dom and appends to container
function add_imperial_board(){
    ...
    new_board_group.innerHTML = `
        ...
        <div class="imperial-grid-container">
            ...
            <div class="dimension-input-area">
                <input type="number" class="number-input" id="length-whole-${get_index}" name="length_whole[]" min="0" required>
                <input type="number" class="number-input" id="width-whole-${get_index}" name="width_whole[]" min="0" required>
                <input type="number" class="number-input" id="height-whole-${get_index}" name="height_whole[]" min="0" required>
            </div>
...
//adds metric board to dom and appends to container
    ...
    new_board_group.innerHTML = `
       <strong><u>Board ${get_index}:</u></strong><br><br>
        ...
        <div class="metric-grid-container">
            ...
            <div class="metric-number-div">
                <!-- LENGTH in centimeters, for example -->
                <input type="number" step="0.01" class="number-input" id="length-metric-${get_index}" name="length_cm[]" min="0">
                <!-- WIDTH in cm -->
                <input type="number" step="0.01" class="number-input" id="width-metric-${get_index}" name="width_cm[]" min="0">
                <!-- HEIGHT in cm -->
                <input type="number" step="0.01" class="number-input" id="height-metric-${get_index}" name="height_cm[]" min="0">
            </div>
        </div>
    `;
...
//function gets length of board groups in container and removes the board at the last index and decrements counter
function remove_last_board(unit_type){

    //check at least 2 board entries
    if(imperial_board_count > 1 && unit_type == "imperial"){
        const board_groups = imperial_container.querySelectorAll('.board-group');
    ...
    if(metric_board_count > 1 && unit_type == "metric"){
        const board_groups = metric_container.querySelectorAll('.board-group');
        const last_index = board_groups.length - 1;
        const last_board_group = board_groups[last_index];

        metric_container.removeChild(last_board_group);
        metric_board_count --;
    }
}
...
function remove_board(){
    if(imperial_radio.checked){
        remove_last_board("imperial");
    }
    else{
        remove_last_board("metric");
    }
}
